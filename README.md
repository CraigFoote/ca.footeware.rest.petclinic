...
1. `sudo mkdir -p /opt/rest.petclinic`
1. `sudo chmod -R 777 /opt/rest.petclinic/`

# Building

1. `mvn clean package` or use the `-BUILD` eclipse launch config
1. `docker run -d --name rest.petclinic -p 10000:10000 --network petclinic-network -v /opt/rest.petclinic/logs:/opt/rest.petclinic/logs -t petclinic:[version]`
